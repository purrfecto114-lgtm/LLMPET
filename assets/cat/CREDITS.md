...
本皮肤（`skin: cat`）使用的猫咪 meme 表情包 GIF，来自「月薪喵」表情包第 1 弹。
...
原文共 62 张表情，本皮肤从中挑选了 14 张，按桌宠状态命名放在本目录：
...
运行时 GIF 由上述同一批署名素材做确定性尺寸处理，没有生成式重画或新增内容：120×120 原图使用边缘感知 xBR 放大 3 倍，原本为 240×240 的 `cat-loafing-3.gif` 使用 Lanczos 缩放到 360×360。所有动画保留原帧数、总时长与透明背景，在桌面端按 180×180 逻辑像素显示，为 Retina 屏提供 2× 像素源。处理脚本为 `scripts/upscale-cat-gifs.sh`。
