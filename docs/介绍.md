...
它通过 Claude Code / CodeWhale 的公开 hook 接口工作，不碰你的账号，也不外发本地会话数据。计量只读取 token、模型与时间戳；消息气泡会读取最后一段回复并在本机截断、清洗、脱敏。唯一的联网动作是每 24h 下载一次公开模型价目表（只下不传，可用 `OCTOPUS_NO_NET=1` 关闭）。
...
| 🔒 | **本地优先** | HTTP 仅绑 `127.0.0.1`；会话数据不外传；回复仅用于本地气泡；钩子安装合并不覆盖、可一键卸载 |
...
- **最小读取、不外传**：计量只读 transcript 里的 token 数、模型名、时间戳；消息气泡只抽最后一段 assistant 文本并做**截断与密钥脱敏**，全程只在 `127.0.0.1` 本机流转。
